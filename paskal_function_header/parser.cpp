...
std::shared_ptr<parser::node> parser::S()
{
    ...
    case token::FUNCTION :
        ...
        parse_exception::parse_assert(lexer_ptr->get_cur_token() == token::LBR || lexer_ptr->get_cur_token() == token::DOTDOT);
        if (lexer_ptr->get_cur_token() == token::DOTDOT) {
    ...
    case token::PROCEDURE :
        ...
        parse_exception::parse_assert(lexer_ptr->get_cur_token() == token::LBR || lexer_ptr->get_cur_token() == token::DOTCOMA);
        if (lexer_ptr->get_cur_token() == token::DOTCOMA) {
            ch6 = std::make_shared<node>(";");
            res.add_children(ch6);
            lexer_ptr->next_token();
